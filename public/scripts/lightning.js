...
var Lightning = (function (window) {
  ...
  var LightningAbstract = {
    ...
    _draw: function (ctx) {
      ...
      ctx.save()
      ctx.beginPath()
      ctx.strokeStyle = this._colorToString(
        Math.random() * (opts.maxAlpha - opts.minAlpha) + opts.minAlpha
      )
      lineWidth =
        Math.random() * (opts.maxLineWidth - opts.minLineWidth) +
        opts.minLineWidth
    ...
    _filterApply: function (
      ...
      // Duplicate the line start point and end point references before and after the array for spline completion.
      ...
      // Remove reference added for completion
      points.pop()
      // Add as starting point of drawing after deleting
  ...
  LChild.prototype = extend(LightningAbstract, {
    ...
    _update: function () {
      ...
      // Update the starting and ending points obtained from the parent at regular intervals or when the number of points in the parent is less than the end step position of the child.
      ...
      if (
        ...
        var stepMin = (plen * 0.1) | 0,
          startStep = (this._startStep =
            (Math.random() * (((plen / 3) * 2) | 0)) | 0)
        this._endStep =
          startStep +
          stepMin +
          ((Math.random() * (plen - startStep - stepMin) + 1) | 0)
        this._lastChangeTime = currentTime
      }

      // Extract the acquisition range from the parent point array
      range = parent.points.slice(this._startStep, this._endStep)
      rangeLen = range.length

      // Get spline curve control points from range
